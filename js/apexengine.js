$(document).ready(function() {
  ...
  db.collection("apex").orderBy("damageTotal", "desc").get().then(function(querySnapshot) {
    querySnapshot.forEach(function(doc) {
      ...
      var teamDamage = doc.data().player1.diff+ doc.data().player2.diff + doc.data().player3.diff;
      $('.progress').hide();
      var teamCard = '<div class="card horizontal transparent z-depth-5 hoverable" style="height: 150px;"><div class="card-image"><img style="border-radius: 15px 0px 0px 15px; height: 150px; width: 200px;" src="' + doc.data().banner + '"></div><div class="card-stacked"><div class="card-content grey darken-4" style="border-radius: 0px 15px 15px 0px; padding: 10px; padding-top: 0px; padding-bottom: 0px;"><div class="row container"><div class="col m6 push-m1"><h5 class="white-text left-align"> Team ' + doc.data().teamName + ' </h5></div><div class="col m6 push-m4"><h5 class="white-text right-align hide-on-small-only"> ' + teamDamage + ' PTS</h5></div></div><div class="row" style="margin-top: -1%;"><div class="col m4 hoverable"><p class="white-text center-align" style="font-size: 15px;"> ' + doc.data().player1.diff + ' PTS</p><p class="white-text center-align" style="font-size: 12px;"> ' + doc.data().player1.name + '</p></div><div class="col m4 hoverable "><p class="white-text center-align" style="font-size: 15px;"> ' + doc.data().player2.diff + ' PTS</p><p class="white-text center-align" style="font-size: 12px;"> ' + doc.data().player2.name + '</p></div><div class="col m4 hoverable"><p class="white-text center-align" style="font-size: 15px;"> ' + doc.data().player3.diff + ' PTS</p><p class="white-text center-align" style="font-size: 12px;"> ' + doc.data().player3.name + '</p></div></div></div></div></div>';
      $('#teamSection').append(teamCard);

      var teamCardMobile = '<div class="card z-depth-3 hoverable"><div class="card-image waves-effect waves-block waves-light"><img class="activator" src="' + doc.data().banner + '" style="border-radius: 15px 15px 0px 0px;"><span class="card-title">Team ' + doc.data().teamName + '</span></div><div class="card-content center-align"><h4 style="font-weight: 900;">' + teamDamage + ' PTS</h4></div><div class="card-reveal"><span class="card-title grey-text text-darken-4">Team ' + doc.data().teamName + '<i class="material-icons right">close</i></span><h5 >' + doc.data().player1.name + ' <br><span style="font-weight: 900;">' + doc.data().player1.diff + '</span></h5><br><h5 >' + doc.data().player2.name + ' <br><span style="font-weight: 900;">' + doc.data().player2.diff + '</span></h5><br><h5 >' + doc.data().player3.name + '<br><span style="font-weight: 900;">' + doc.data().player3.diff + '</span></h5><br></div></div>';
      $('#teamSectionMobile').append(teamCardMobile);
      // var teamCardMedium = '<div class="card z-depth-3 hoverable"><div class="card-image waves-effect waves-block waves-light"><img class="activator" src="'+doc.data().banner+'" style="border-radius: 15px 15px 0px 0px;"><span class="card-title">Team '+doc.data().teamName+'</span></div><div class="card-content center-align"><h4 style="font-weight: 900;">'+teamDamage+' PTS</h4></div><div class="card-reveal"><span class="card-title grey-text text-darken-4">Team '+doc.data().teamName+'<i class="material-icons right">close</i></span><h5 >'+doc.data().players[0].name+' <br><span style="font-weight: 900;">'+doc.data().players[0].difference+'</span></h5><br><h5 >'+doc.data().players[1].name+' <br><span style="font-weight: 900;">'+doc.data().players[1].difference+'</span></h5><br><h5 >'+doc.data().players[2].name+'<br><span style="font-weight: 900;">'+doc.data().players[2].difference+'</span></h5><br></div></div>';
      // $('#teamSectionMedium').append(teamCardMedium);
  ...
  db.collection("apex").get().then(function(querySnapshot) {
    ...
    $("#arenaTeams").replaceWith('<h3 class="white-text center-align hide-on-medium-only" id="lobbyPlayers">' + teamCounter + ' Teams</h3>');
  });
